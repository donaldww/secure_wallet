...
func (e *Enclave) storeChecksum(checksum *memguard.LockedBuffer) {
	e.Enclave = checksum.Seal()
}

func (e *Enclave) securePeriodicCheck(filePath string) {
	for {
		...
		if !e.secureCompareChecksum(currentChecksum) {
			fileStatus = "altered"
		} else {
			fileStatus = "intact"
		}
...
func (e *Enclave) secureCompareChecksum(currentChecksum *memguard.LockedBuffer) bool {
